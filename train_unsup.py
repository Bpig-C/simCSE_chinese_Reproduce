...
from networks import SimCSEModelUnsup
...
# 加载数据集
datasets_sts = {
    'STS-B-train': load_data_new('datasets/STS-B/STS-B.train.data'),
    'STS-B-test': load_data_new('datasets/STS-B/STS-B.test.data'),
    'STS-B-valid': load_data_new('datasets/STS-B/STS-B.valid.data')
}
datasets_lqcmc = {
    'LCQMC-train': load_data_new('datasets/lcqmc/train.tsv'),
    'LCQMC-test': load_data_new('datasets/lcqmc/dev.tsv'),
    'LCQMC-valid': load_data_new('datasets/lcqmc/test.tsv')
}
...
def train(model, train_dl, dev_dl, optimizer) -> None:
    ...
    for batch_idx, source in enumerate(tqdm(train_dl), start=1):
        ...
        if batch_idx % 10 == 0:     
            ...
            if best < corrcoef:
                best = corrcoef
                torch.save(model.state_dict(), hp.SAVE_PATH_UNSUP)
...
if __name__ == '__main__':
    ...
    logger.info(f'Train is finished, best model is saved at {hp.SAVE_PATH_UNSUP}')
